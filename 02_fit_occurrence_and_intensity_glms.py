...
"""
02_fit_two_stage_glm_STRICT_equation_pdf.py

Purpose
-------
Fit the exact two-stage rainfall GLM written in equation.pdf.
...
Stage 1: Occurrence model
    ...
    eta_occ(t) = beta0
               + sum_{k=1}^{2} [alpha_k sin(omega_k d_t) + gamma_k cos(omega_k d_t)]
               + sum_{j=1}^{5} phi_j W_{t-j}
               + delta N5_w(t)
               + theta' X_t

    X_t = [nino34_t, dmi_t, mjo_x_t, mjo_y_t, year_c_t,
           nino34_t sin(omega_1 d_t), nino34_t cos(omega_1 d_t),
           dmi_t sin(omega_1 d_t),    dmi_t cos(omega_1 d_t)]

Stage 2: Intensity/amount model conditional on wet days
    Y_t = P_t | P_t > 0.1 mm
    Y_t ~ Gamma(mu_t, phi)
    log(mu_t) = eta_amt(t)

    eta_amt(t) = gamma0
               + sum_{k=1}^{2} [a_k sin(omega_k d_t) + b_k cos(omega_k d_t)]
               + sum_{j=1}^{4} lambda_j log(1 + P_{t-j})
               + rho S5_p(t)
               + psi' Z_t

    Z_t = [nino34_t, dmi_t, mjo_x_t, mjo_y_t,
           nino34_t cos(omega_1 d_t), nino34_t sin(omega_1 d_t),
           dmi_t cos(omega_1 d_t),    dmi_t sin(omega_1 d_t)]

Important
---------
This script intentionally removes optional model-selection scenarios, piecewise trend models,
and Stage-2 year trend variants so that the implemented Python model matches equation.pdf.

Input
-----
A daily CSV table containing at least:
    date, rain_mmday
and preferably climate drivers:
    nino34, dmi, mjo_x, mjo_y
or MJO alternatives:
    mjo_phase and mjo_amp, OR rmm1 and rmm2

The script recomputes all equation terms by default, including wet threshold, harmonics,
lags, previous-5-day counts/sums, year_c, and driver-season interactions.

Example Ubuntu command
----------------------
python 02_fit_two_stage_glm_STRICT_equation_pdf.py \
  --input /home/wasiq/Desktop/new1/data/output/step3/glm_input_daily_REGION_1981_2024.csv \
  --output-dir /home/wasiq/Desktop/new1/data/output/strict_equation_glm \
  --tau-mm 0.1
"""
